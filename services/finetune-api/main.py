"""FineTune Team API - SageMaker wrapper for LoRA fine-tuned models."""
...
async def lifespan(app: FastAPI):
    ...
    if endpoint_name:
        app.state.sagemaker = SageMakerClient(
            ...
            timeout_ms=int(os.getenv("SAGEMAKER_TIMEOUT_MS", "60000")),
...
app = FastAPI(
    title="FineTune API",
    description="Fine-tuning team SageMaker wrapper",
    lifespan=lifespan
)

# Setup telemetry
tracer, meter = setup_telemetry(app, "finetune-api", "finetune")
...
async def startup():
    ...
    if await app.state.health.startup_check():
        return {"status": "started", "service": "finetune-api"}
...
async def ready():
    ...
    if await app.state.health.readiness_check():
        return {"status": "ready", "service": "finetune-api"}
...
async def predict(
    ...
    try:
        ...
        with GenAISpanContext(
            ...
            variant_type="lora",
            variant_id=os.getenv("MODEL_VARIANT_ID", "finetune-lora-v1"),
        ...
        # Record metrics
        predict_counter.add(1, {"status": "success", "team": "finetune"})
        predict_latency.record(latency_ms, {"team": "finetune"})
        ...
        predict_counter.add(1, {"status": "timeout", "team": "finetune"})
        sagemaker_error_counter.add(1, {"team": "finetune", "error_type": "timeout"})
        raise HTTPException(
            status_code=504,
            detail={
        ...
        predict_counter.add(1, {"status": "error", "team": "finetune"})
        sagemaker_error_counter.add(1, {"team": "finetune", "error_type": type(e).__name__})
        raise HTTPException(
            status_code=500,
            detail={
